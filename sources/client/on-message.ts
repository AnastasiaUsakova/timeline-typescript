import { startGame, changePlayer, endGame, setSendMessage, takeCard } from './game.js';
...
function processMessage( sendMessage: SendMessage, message: AnyServerMessage ): void
{
	switch ( message.type )
	{
		case 'gameStarted':
			startGame();
			changePlayer( message.myTurn, message.cards, [message.initCard] );
			break;
		
		case 'changePlayer':
			changePlayer( message.myTurn, message.cards, message.deckCards );
			break;
